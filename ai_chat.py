...
try:
    ...
    if DATABASE_URL:
        # Initial connection to ensure table exists
        db_conn_init = psycopg2.connect(DATABASE_URL)
        db_conn_init.autocommit = True
        with db_conn_init.cursor() as cur:
            cur.execute("""
...
def load_global_memory():
    ...
    if not db_enabled or memory_loaded:
        return
    try:
...
async def stream_response(user_id: int, prompt: str, initial_message: discord.Message) -> str:
    ...
    if db_enabled and not memory_loaded:
